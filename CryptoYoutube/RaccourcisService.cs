...
{
    public static class RaccourcisService
    {
        ...
        {
            if ((nCode >= 0) && (wParam == (IntPtr)WM_KEYDOWN) && ((Keys)Marshal.ReadInt32(lParam) == Keys.F8))
            {
                var altgr = GetKeyState((int)Keys.Alt) < 0;
                OnKeyPress(!altgr);
            }
